...
setup: check-macos brew ollama ollama-start models venv pip init index ## Complete setup
	@echo ""
	@echo "$(_GREEN)$(_BOLD)[OK] Setup complete!$(_RESET) (model: $(MODEL))"
...
check-macos: ## Verify running on macOS
	...
	@echo "$(_GREEN)[OK]$(_RESET) macOS detected ($$(sw_vers -productVersion))"

brew: ## Install Homebrew if missing
	@if command -v brew >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) Homebrew already installed"; \
	else \
...
ollama: brew ## Install Ollama via Homebrew if missing
	@if command -v ollama >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) Ollama already installed ($$(ollama --version 2>/dev/null || echo 'unknown'))"; \
	else \
...
ollama-start: ollama ## Start Ollama service
	@if curl -sf $(OLLAMA_URL)/api/tags >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) Ollama is already running"; \
	else \
		...
		for i in $$(seq 1 30); do \
			if curl -sf $(OLLAMA_URL)/api/tags >/dev/null 2>&1; then \
				echo "$(_GREEN)[OK]$(_RESET) Ollama is ready"; \
				break; \
			fi; \
			sleep 1; \
			if [ $$i -eq 30 ]; then \
				echo "$(_RED)[FAIL] Ollama did not start after 30s. Try manually: ollama serve$(_RESET)"; \
				exit 1; \
			fi; \
		done; \
	fi
...
models: ollama-start ## Pull the selected model + embeddings (interactive)
	@if [ "$(MODEL)" = "qwen3:4b" ] && [ -z "$(MAKEFLAGS:M--no-print-directory=)" ]; then \
		echo ""; \
		...
		echo "$(_GREEN)[OK]$(_RESET) $$selected ready"; \
	else \
		echo "$(_CYAN)->$(_RESET) Pulling $(MODEL)..."; \
		ollama pull $(MODEL); \
		echo "$(_GREEN)[OK]$(_RESET) $(MODEL) ready"; \
	fi
	@echo "$(_CYAN)->$(_RESET) Pulling $(MODEL_EMBED) (embeddings for RAG)..."
	@ollama pull $(MODEL_EMBED)
	@echo "$(_GREEN)[OK]$(_RESET) $(MODEL_EMBED) ready"

venv: ## Create Python virtual environment
	@if [ -d "$(VENV)" ]; then \
		echo "$(_GREEN)[OK]$(_RESET) Virtual environment already exists"; \
	else \
		echo "$(_CYAN)->$(_RESET) Creating virtual environment..."; \
		$(PYTHON) -m venv $(VENV); \
		echo "$(_GREEN)[OK]$(_RESET) Virtual environment created at $(VENV)/"; \
	fi
...
status: ## Check status of all dependencies
	...
	@echo "-------------------------------------------"
	@if [ -f "$(PY)" ]; then \
		echo "$(_GREEN)[OK]$(_RESET) Python venv     $$($(PY) --version)"; \
	else \
		echo "$(_RED)[FAIL]$(_RESET) Python venv     not created (run: make venv)"; \
	fi
	@if [ -f "$(PY)" ] && $(PY) -c "import fastapi" 2>/dev/null; then \
		echo "$(_GREEN)[OK]$(_RESET) Python deps     installed"; \
	else \
		echo "$(_RED)[FAIL]$(_RESET) Python deps     not installed (run: make pip)"; \
	fi
	@if command -v ollama >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) Ollama binary   installed"; \
	else \
		echo "$(_RED)[FAIL]$(_RESET) Ollama binary   not found (run: make ollama)"; \
	fi
	@if curl -sf $(OLLAMA_URL)/api/tags >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) Ollama service  running at $(OLLAMA_URL)"; \
	else \
		echo "$(_RED)[FAIL]$(_RESET) Ollama service  not running (run: make ollama-start)"; \
	fi
	@echo "-------------------------------------------"
	...
	@if curl -sf http://localhost:$(PORT)/api/health >/dev/null 2>&1; then \
		echo "$(_GREEN)[OK]$(_RESET) AgenticTM       running at http://localhost:$(PORT)"; \
	else \
...
nuke: clean ## Remove venv + data (keeps knowledge_base & config)
	...
	@rm -rf $(VENV)
	@rm -rf data/
	@echo "$(_GREEN)[OK]$(_RESET) Nuked. Run 'make setup' to rebuild."
